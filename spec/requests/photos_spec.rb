  ...
  describe 'POST /photos' do
    ...
    context 'when image and caption are provided' do
      ...
      it 'returns correct JSON response with correct image filename being ID of the photo' do
        json_response = JSON.parse(response.body).symbolize_keys
        id = json_response[:id]
        expect(json_response[:avatar]).to match(/#{id}\.jpg$/)
      end
  ...
  describe 'GET /photos' do
    context 'when user has multiple photos' do
      it 'returns them sorted by ID' do
        post '/photos', params: { photo: { caption: 'River', image: fixture_file_upload('296x277.jpg') } }
        expect(response).to have_http_status(201)

        post '/photos', params: { photo: { caption: 'Forest', image: fixture_file_upload('545x307.jpg') } }
        expect(response).to have_http_status(201)

        post '/photos', params: { photo: { caption: 'Field', image: fixture_file_upload('850x500.jpg') } }
        expect(response).to have_http_status(201)
        ...
        expect(json_response[0]).to include(caption: 'River', id: 1, avatar: /1\.jpg$/)
        expect(json_response[1]).to include(caption: 'Forest', id: 2, avatar: /2\.jpg$/)
        expect(json_response[2]).to include(caption: 'Field', id: 3, avatar: /3\.jpg$/)
      end
    end
  end
end
